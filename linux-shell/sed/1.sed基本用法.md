...
* 操作
    ...
    * 将 /etc/paaswd内容 + 行号重定向至 /tmp/passwd
...
* 行号过滤
    * 正过滤语法
        ...
        * 第N行至末尾行数据: ``` sed -n '30,$p' /tmp/passwd ```
        * 起始行号+步长行数据: ```sed -n '3~3p' /tmp/passwd```
        * 起始行号+后续N行数据: ``` sed -n '10,+5p' /tmp/passwd ```
    * 反过滤语法
        * 除去第N行外 行数据:``` sed -n '3!p' /tmp/passwd ```
...
* 行号+正则过滤
    ...
    * 示例代码：查找1～1500行内以 icmp 或 igmp 开始的行数据
        ```
        [zhui@centos7-vm ~]$ sed -rn '1,1500{/^(icmp|igmp)/p}' /etc/services 
        igmpv3lite      465/udp                 # IGMP over UDP for SSM 
        ```
    * 思考题🤔：查找1500行至文档末尾中，以 icmp 或 igmp 开始的行数据
...
* 根据正则表达式删除
    * 删除以bash结尾的行数据： ``` sed -i.bak '/bash$/d' /tmp/passwd ```
    * 思考题🤔：
        * 删除用户名中带字符o的行数据？
        * 提示： 文件中的第一个冒号前的是用户名
    ...
    * 匹配行数据的前一行插入数据(insert)
        ...
        * 正则匹配：在localhost6的前一行插入数据
            * 示例代码：
                ...
                ::1         localhost localhost.localdomain localhost6 localhost6.localdomain6
                ```
            * 思考题🤔：假如/tmp/hosts有多行数据包含localhost6，那么运行结果会有怎样？
...
* 替换行数据
    ...
    * 替换正则匹配的行数据，统一修改包含localhost6的行数据：
        ```
...
* 替换字符串
    ...
    * 正则过滤替换
        ...
        * 位置对掉
            ...
            ```
        * 思考题🤔
            * 将 test.txt中的 ```123 456 789``` 对掉成 ``` 789 456 123 ```
            * 将 /tmp/passwd 中每行的用户名统一改成nobody
